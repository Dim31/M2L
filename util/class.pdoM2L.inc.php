...
{   		
    ...
    {
        $reqmotifs = "Select libelle, id_motif from motifs";
        $resmotifs = PdoM2L::$monPdo->query($reqmotifs);
        $lesmotifs = $resmotifs -> fetchAll();
        
        return $lesmotifs;
    }

    public function getlelibelle($lemotif)
    {
        $reqmotifs = "select libelle from motifs WHERE id_motif = $lemotif";
        $resmotifs = PdoM2L::$monPdo->query($reqmotifs);
        $lelibelle = $resmotifs -> fetchAll();
